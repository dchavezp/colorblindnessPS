    ...
    public static void labMethod(String imageName, String savePath, double factor) {
        ...
        for (int i = 0; i < height; i++) {
            for (int j = 0; j < width; j++) {
                Color myColor = new Color(img.getRGB(j, i)); // mycolor saca rgb de cada posicion de la imagen

                lab = rgbToLab(myColor.getRed(), myColor.getGreen(), myColor.getBlue());//lab almacena cada valor del rgb r g b

                if(lab[1] < 0){ // procesa el valor de la banda verde
                    lab[1] = lab[1] * -1;
                ...
                if(lab[2] < 0){ // procesa en valor de la banda azul
                ...
                int rgb[] = new int[3];
                rgb = labToRgb(lab[0],lab[1] * 128 ,lab[2] * 128); // nueva matriz con nuevos valore de las bandas

                System.out.println(rgb[0] + "," + rgb[1] + "," + rgb[2]);

                if (rgb[0] < 0){ // convierte la banda roja a 0 o negro
                    rgb[0] = 0;
                } else if(rgb[0]>252){
                    rgb[0] = 252;
                }
                ...
                int rgbColor = newColor.getRGB();// valor rgb de las 3 bandas modificadas

                img.setRGB(j,i,rgbColor); // cambia el pixel con el nuevo varlor de la imagen

            }
        }

        saveImage(savePath, img); // guarda la imagen en u bojeto temporal	
